...
import com.epam.libraryservice.exceptions.LoginFirstException;
...
//		return book.stream().map(bookId->feignService.getBookByBookId(bookId).getBody()).collect(Collectors.toList());
	...
	public void validate(String token) {
		if(Boolean.TRUE.equals(feignService.verifyToken(token).getBody()))
		{
			throw new LoginFirstException("login first");
		}
	}
