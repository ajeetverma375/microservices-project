...
class LibraryControllerTest {
	...
	@BeforeEach
	public void setUp() {
		userEntity = new UserEntity("Ajeet@123", "ajeet@gmail.com", "ajeet","pass");
...
//	}
	...
	@Test
	void testGetLibraryRecordByUsername() throws Exception {
		List<LibraryEntity> libraryList = new ArrayList<>();
		libraryList.add(new LibraryEntity(1, "Ajeet@123", 3));
		libraryList.add(new LibraryEntity(2, "Akash@123", 2));
		when(libraryService.getAllBookByUserName("Ajeet@123")).thenReturn(libraryList);
		mockMvc.perform(get("/library/Ajeet@123").content("Ajeet@123")).andExpect(status().isOk());
	}

	@Test
	void testIsseueBookToUser() throws Exception {
		String username = "Ajeet@123";
		when(libraryService.issueBookToUser(username, 1)).thenReturn("book issued successfully to: " + username);
		mockMvc.perform(post("/library/users/Ajeet@123/books/1").content("Ajeet@123").content("1"))
				.andExpect(status().isOk());
	}

	@Test
	void testReleaseBook() throws Exception {
		when(libraryService.deleteIssuedBook("Ajeet@123", 1)).thenReturn(true);
		mockMvc.perform(delete("/library/users/Ajeet@123/books/1").content("Ajeet@123").content("1"))
				.andExpect(status().isOk());
	}
